...
public class PlayerMiscTrigger implements Listener {
    ...
    private void execute(Player player, ActionTrigger trigger, org.bukkit.event.Event event) {
    ...
    public void onTeleport(PlayerTeleportEvent event) {
        execute(event.getPlayer(), ActionTrigger.ON_TELEPORT, event);
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onExpChange(PlayerExpChangeEvent event) {
        execute(event.getPlayer(), ActionTrigger.ON_EXP_CHANGE, event);
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onLevelChange(PlayerLevelChangeEvent event) {
        execute(event.getPlayer(), ActionTrigger.ON_LEVEL_CHANGE, event);
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onBedEnter(PlayerBedEnterEvent event) {
        execute(event.getPlayer(), ActionTrigger.ON_BED_ENTER, event);
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onBedLeave(PlayerBedLeaveEvent event) {
        execute(event.getPlayer(), ActionTrigger.ON_BED_LEAVE, event);
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onBucketFill(PlayerBucketFillEvent event) {
        execute(event.getPlayer(), ActionTrigger.ON_BUCKET_FILL, event);
    }

    @EventHandler(priority = EventPriority.HIGHEST)
    public void onBucketEmpty(PlayerBucketEmptyEvent event) {
        execute(event.getPlayer(), ActionTrigger.ON_BUCKET_EMPTY, event);
    }
